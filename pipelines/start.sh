...
# --- 新增/修改的部分：设置 PYTHONPATH ---
# 获取当前脚本的绝对路径
SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" &> /dev/null && pwd)
# 假设 agent1_package 位于 Open WebUI 根目录，即 pipelines 目录的上一级
OPENWEBUI_ROOT_DIR=$(dirname "$SCRIPT_DIR")

# 将 Open WebUI 根目录添加到 PYTHONPATH
# 这样 Python 就能找到 agent1_package 了
export PYTHONPATH="${OPENWEBUI_ROOT_DIR}:${PYTHONPATH}"

echo "PYTHONPATH set to: ${PYTHONPATH}" # 打印出来确认
# --- 新增/修改的部分结束 ---
